        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                if app.sourceAssets.count < 3 {
                    InfoBanner(text: app.connectionState == .demo ? "建议上传至少 3 个角度；演示模式选择 1 张即可继续。" : "还需上传 \(3 - app.sourceAssets.count) 张，真实生成要求至少 3 个角度。", kind: .warning)
                }
                ...
                PrimaryButton(
                    ...
                    icon: "sparkles",
                    isLoading: isWorking,
                    disabled: app.productName.trimmed.isEmpty || app.sellingPoint.trimmed.isEmpty || app.sourceAssets.count < (app.connectionState == .demo ? 1 : 3)
