    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                if app.connectionState == .preview { InfoBanner(text: "预览模式沿用你上传的参考图展示审核交互，不代表图片已由 AI 生成。", kind: .warning) }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                InfoBanner(text: "保存时会同时保留通用 MP4。首版不提供 Android 动态照片入口。", kind: .neutral)
