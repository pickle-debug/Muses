    ...
    var body: some View {
        AssetImage(asset: asset, contentMode: .fit)
            ...
            .overlay(alignment: .topTrailing) {
                StatusPill(text: app.connectionState == .preview ? "安全预览" : "3:4 生成结果", tone: app.connectionState == .preview ? .warning : .success)
                    .padding(14)
            }
            .accessibilityLabel(app.connectionState == .preview ? "商品安全预览" : "生成商品图片")
    }
}
            ...
            if let player {
                ...
                VStack(spacing: 12) {
                    Image(systemName: "play.circle.fill")
                        ...
                        .shadow(radius: 8)
                    Text(app.connectionState == .preview ? "预览模式无真实视频" : "正在加载视频")
