...
- 首批仅邀请 3–5 名用户，不开放注册。
- Demo 使用原生 SwiftUI，仅交付 iOS 17+ iPhone 版本；不使用 React Native 或 Expo。
- Demo 只适配小红书，不做抖音。
- Demo 不做登录、账号体系、服务端业务系统和埋点。
- Demo 直接从客户端请求固定供应商 Endpoint；每位用户使用独立临时 Key。
- 图片默认使用 `gpt-image-2`；图片、视频、文本模型名由本地 JSON 配置。
- 每个创作任务只生成一张 3:4 图片和一段 3–5 秒、720p、静音的 AI 动态。
- Demo 输出 Apple Live Photo、MP4 和静态图；Android Motion Photo 移至四周 MVP 的兼容性 Spike。
- 所有最终内容必须经过商品一致性人工审核。
- 内容由用户保存并人工发布，不接小红书 OAuth 或自动发布。
- Demo 免费；四周内测成本总上限为 100 元。
...
Demo 不做自动埋点，由产品负责人通过直接访谈记录。四周 MVP 是否加入最小匿名统计，在内测后另行决定。
...
- 若四周内启动 Android Spike，再记录 Motion Photo 相册识别率和小红书发布成功率；该指标不属于 Demo 漏斗。
...
Demo 只提供“真人手持生活方式”模板，用户不直接编辑底层完整提示词。
...
### 13.2 Android（四周 MVP Spike，不属于 Demo）

- 仅做格式与目标机型验证，不在 SwiftUI Demo 中向用户展示“生成动态照片”。
...
### 13.3 Android 兼容测试（立项条件：iOS Demo 获得发布/复用反馈）
...
- Demo 无平台帖子 ID，不得自动记为 `published`。
...
用户已明确选择 Demo 不建设后端，并将每人独立临时 Key 保存在普通本地存储中。该方案有凭证被设备备份、调试工具、恶意环境或越狱/Root读取的风险，只允许用于受控 Demo。
...
- 达到预算 80% 时暂停新增用户并复盘。
- 不提供每日重置免费额度。
- 即使 Demo 没有自动统计，也要通过供应商后台和 Key 配额核对实际成本。
...
- Android 不作为 Demo 上线门槛；四周内是否执行 Motion Photo Spike 由 iOS 用户反馈决定。
...
| 风险 | 影响 | 应对 |
|---|---|---|
| 2–3 日范围过大 | Demo 无法按时跑通 | 固定单模板、单图、单动态、单平台、无账号 |
| 直接客户端调用导致 Key 泄露 | 被刷量和产生费用 | 一人一 Key、限额、短有效期、固定域名、结束撤销 |
| 商品失真 | 货不对板、用户不敢发布 | 商品快照、并排审核、失败标签、禁止强制保存 |
| SwiftUI Demo 无法覆盖 Android 用户 | 首批可测试用户减少 | 先验证 iOS 发布价值；四周内以 Motion Photo Spike 决定原生 Android 或 RN 投入 |
...
- 2–3 个音节，适合中文口语传播。
- 能拟人化为一个亲切的 AI 营销角色。
- 具有可解释的中国文化背景，但不过度高雅或有距离感。
- 能承载 AIGC、营销、商品表达与未来多平台能力。
- 避免“爆单、神器、暴卖”等短期和夸大表达。
- 正式确定前完成商标、App Store、安卓商店、域名和社交账号重名检索。

在正式命名完成前，研发、配置、Bundle 显示名和文档统一使用开发代号 `Muses`。
